...
它也會先閱讀專案裡留下的架構筆記，例如某個功能應該自己管理哪些資料、哪些地方不能直接互相連接。就算你換了一個 AI 工具、開了新的對話，重要決定仍然留在專案裡，不必每次從頭解釋。

如果這次修改真的改變了重要分工，AI 也會同步更新最接近的架構筆記；如果只是做一個小按鈕，它不會為了看起來專業就到處新增文件。
