...
若要一次全域安裝三個 skills，執行：

```sh
npx skills@latest add Rayyyyyyyyyyyyyyyyyyyy/ai-coding-architecture-skill --skill '*' --global
```

這會預先選好 `coding-architecture`、`architecture-context` 和 `safe-change`，再讓你選擇要安裝到哪些 AI 工具，例如 Codex、Claude Code 或其他支援 Agent Skills 的工具。若只想安裝到目前專案，移除 `--global`。安裝後開啟新的對話；如果 skills 沒有出現，請重新啟動工具。
