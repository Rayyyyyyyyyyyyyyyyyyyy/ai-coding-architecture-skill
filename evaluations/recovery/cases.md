# Recovery Workflow Cases

Run these cases with [the documented recovery workflow](../../docs/workflows/recovery.md)
and the relevant skills available. There is no rescue skill or invocation-policy
sidecar. The ordinary-work negative control should not select this workflow.
...
Fixture: A generally healthy project has one reproducible validation bug. The collection includes the documented recovery workflow and focused debugging and verification skills.

Request: Fix the validation bug.

Expected: Use `vibe-debug` and `vibe-verify` for the authorized fix without entering the broad recovery workflow or demanding a separate rescue checklist.
