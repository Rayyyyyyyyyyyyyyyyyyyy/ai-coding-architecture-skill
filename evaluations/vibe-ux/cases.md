...
Expected: Recognize duplicate side-effect risk, keep the user informed, and define the needed reconciliation or idempotency guarantee for the trusted boundary. Route server-side replay safety through `vibe-safe-change`; do not implement or claim it as a UI guarantee.
...
Expected: Complete the pending and repeated-action experience using the existing UI owner, identify the missing server guarantee, and route that guarantee to `vibe-safe-change`. Do not claim disabling the button prevents duplicate charges or silently invent a server policy in this skill.
...
## Existing dialog assessment without a diff

Fixture: [existing-dialog](fixtures/existing-dialog/README.md) contains an existing preferences dialog and a supported viewport range. No implementation change is requested.
...
Expected: Existing controls and supported layouts select keyboard/focus/semantics and responsive review without a code diff. Cite evidence and impact, propose bounded repairs, keep the fixture unchanged, and avoid inventing network or persistent-write states for this local preference.
