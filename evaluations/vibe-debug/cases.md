...
Expected: Establish the same red-capable loop, isolate the cause, make the smallest fix at the owning boundary, and rerun the loop. Then invoke `vibe-verify` for post-fix evidence selection and the completion claim rather than copying a generic verification checklist into debugging.
