...
For an authorized implementation being finished, use the operative cross-skill invocation in [SKILL.md](../SKILL.md) and provide the relevant risk outcome, such as denied authorization behavior, migration compatibility with representative existing data, or absence of privileged values from an untrusted artifact. `vibe-verify` owns concrete check selection and the completion claim. Local implementation evidence never implies production migration, secret provisioning, or external configuration occurred.
