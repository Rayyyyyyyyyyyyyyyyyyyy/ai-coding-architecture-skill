...
# 接手故障專案 / Recovery workflow

Use this documented workflow when the user explicitly asks to stabilize a broken
project. A partially built repository can start with `$vibe-project` for understanding
or assessment; unfinished work alone does not require recovery. This document is
not a discoverable skill and does not add an invocation name.
...
Propose the smallest vertical slice that restores a user outcome or reliable development baseline. Ask the user to confirm or adjust the checklist and wait for an explicit response. Selecting this workflow, a general request to rescue the project, or silence does not approve the proposed items. If no actionable issue is found, report that result instead of inventing a repair.
...
| --- | --- |
| causal failure investigation | `vibe-debug` |
| dependency/trust/auth/data/runtime/destructive risk | `vibe-safe-change` |
| non-trivial frontend boundary | `vibe-frontend` |
| relevant journey states | `vibe-ux` |
| durable architectural intent | `vibe-design-context` |
| exit verification | `vibe-verify` |
...
On actual agent transfer invoke `vibe-handoff`. Report the verified slice and remaining boundaries, not a code-smell catalog or whole-repository health claim.
